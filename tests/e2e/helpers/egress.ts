...
/**
 * T036 + T137 / FR-048, SC-010, Principle I — the egress watcher.
 *
 * **This lives in `helpers/` rather than beside the suite that grew it, and that move
 * fixed a broken gate.** Six spec files import `watchForEscapes`, and Playwright
 * refuses to load a test file that imports another test file — so while each spec
 * passed when run on its own, `npm run test:e2e` collected **zero tests**. The main
 * end-to-end gate had been silently empty since the first spec reached for this
 * helper. It is shared deliberately, so the suites cannot drift apart from the
 * definition of "escaped"; it just cannot be shared out of a `*.spec.ts`.
 *
